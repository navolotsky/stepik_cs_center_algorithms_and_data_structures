...
class BinaryTree:
    ...
    def is_correct(self):
        prev = None
        for cur in self.in_order_iter_by_iterative():
            if not (
                (prev is None or prev._key <= cur._key) and
                (cur._left is None or cur._left._key < cur._key) and
                (cur._right is None or cur._key <= cur._right._key)
            ):
                return False
            prev = cur
        return True
...
def test():
    ...
    assert BinaryTree.build_from_list(
        [[2147483647, -1, -1]]).is_correct() == True
    assert BinaryTree.build_from_list([
        [2, 1, 2],
        [2, -1, -1],
        [3, -1, -1]
